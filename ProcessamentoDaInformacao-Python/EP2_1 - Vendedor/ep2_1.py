...
def calculo_comissao(salario, vendas):
    return salario + (vendas * 0.2)

def main():
    salario = float(input())
    vendas = float(input())

    salario_total = calculo_comissao(salario, vendas)
    print("%.2f"%(salario_total - salario))

    if salario_total >= salario * 1.5:
